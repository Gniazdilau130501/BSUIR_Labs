...
void printMenu() {
	...
	std::cout << "-> Тыкни \"E\",чтобы завершить выполнения этой шедевральной программы" << std::endl;
	std::cout << "-> Тыкни цифру устройства, которое надо безопасно извлеч" << std::endl;
	std::cout << "Что сейчас подключено по USB:" << std::endl << std::endl << std::endl;
}
